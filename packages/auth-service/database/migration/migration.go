...
func CreateMigrationFile(migrationFileName string) error {
	...
	migrationsPath := wd + "/database/migration/migration_files/"
...
func Migrate(migrationFileName string) error {
	...
	err = conn.Raw("SELECT migrations FROM migrations where deleted_at is null").Find(&migrateTable).Error
	if err != nil {
		return err
	}
...
func executeSQLQueryFile(wd string, migrationFileName string, conn *gorm.DB) error {
	mFile, err := os.Open(wd + "/database/migration/migration_files/" + migrationFileName)
	if err != nil {
		defer os.Exit(1)
		return err
	}
